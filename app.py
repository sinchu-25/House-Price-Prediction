...
def load_artifacts():
    ...
    return model, scaler, encoder, feature_columns, skewed_features, template_row, metrics, neighborhoods

model, scaler, encoder, feature_columns, skewed_features, template_row, metrics, neighborhoods = load_artifacts()
...
row.update({
    ...
    # Fields the engineering step needs alongside YearBuilt
...
def predict_price(raw_row_df):
    ...
    # Target encode the high-cardinality columns using the fitted encoder
    present = [c for c in TARGET_ENC_FEATURES if c in df.columns]
    df[present] = encoder.transform(df[present])

    # Align to the exact training column set/order
    df = align_columns(df, feature_columns)

    # Apply the same skew correction used at training time
